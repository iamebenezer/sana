...
(function($) {
    ...
    $.snow = function(options) {
        ...
        var defaults = {
            ...
            'flake_folder': 'bubble-26/', // folder with flake images snowflake-%d.gif
        ...
        for (var i = 0; i < options.flake_number; i++) {
            ...
            if (typeof(options.flake_folder) == 'object') {
                ...
                flake_elem = '<img src="' + pssnoweffect.plugin_url + options.flake_folder + 'snowflake-' + (Math.floor(random() * options.flake_imgs) + 1) + '.png' + '" style="border:0!important; background-color:transparent!important; margin:0!important; padding:0!important;" width="' + flake_size + '" height="' + flake_size + '" />';
            }
                ...
                for (var i = 0; i < options.flake_number; i++) {
                    ...
                    if (yp[i] > options._window_height - flake_size * 0.75) {
                        ...
                        if (options.rotation != 0) {
                            ...
                            degree[i] %= 360;
                            flakes[i].css({
                                    WebkitTransform: 'rotate(' + degree[i] + 'deg)'
                                })
                                .css({
                                    '-moz-transform': 'rotate(' + degree[i] + 'deg)'
                                })
                                .css({
                                    "transform": "rotate(" + degree[i] + "deg)"
                                })
                                .css({
                                    "-o-transform": "rotate(" + degree[i] + "deg)"
                                })
                                .css({
                                    "-ms-transform": "rotate(" + degree[i] + "deg)"
                                });
                        } // if rotation
                    } // if in screen
                } // for flakes
