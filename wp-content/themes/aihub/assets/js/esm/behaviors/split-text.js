...
class r extends l {
    ...
    get willEmitEvents() {
        return ["change:animatableElements"]
    }
    get appEvents() {
        return {
            "after:windowResize": {
    ...
    async initialize() {
        this.isDestroyed || (this.getOption("splitDoneFromBackend") || (this.widthBeforeResize = await fastdom.measure(() => this.view.el.offsetWidth), await document.fonts.ready), this.isDestroyed) || (this.elements = this.getElements(), this.beginSplit())
    }
    getElements() {
    ...
    setSplitUnitsAttrs(t) {
        const e = t === "chars,words" ? "words" : t,
            i = this.splitText[e];
        i.forEach((s, n) => {
            s.style.setProperty("--split-text-index", n), s.style.setProperty("--split-text-last-index", i.length - 1 - n)
        })
    }
